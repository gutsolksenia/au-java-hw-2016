...
public class Server implements AutoCloseable {
    ...
    private void getListOfFiles(Connection curConnection) throws IOException {
        ...
        ArrayList<FileHolder> myFiles = new ArrayList<>();
        if (files == null) {
            curConnection.writeList(0, myFiles);
            return;
        }
        for (File file : files) {
            myFiles.add(new FileHolder(file.getName(), file.isDirectory()));
        }
        curConnection.writeList(files.length, myFiles);
    }
}